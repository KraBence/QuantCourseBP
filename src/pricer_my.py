            ...
            if updown == UpDown.DOWN:
                ...
                if inout == InOut.OUT:
                    ...
                    price_dic = direction * spot * (barrier / spot) ** (2 * rate / vol ** 2) * \
                                BarrierAnalyticPricer.bs_call(barrier / spot, strike / barrier, vol, rate,
                                                              time_to_expiry, df)
...
class BarrierBrownianBridgePricer(Pricer):
    ...
    def calc_fair_value_with_ci(self) -> float:
        ...
        fv = mean(path_payoff) * self.model.calc_df(maturity)
        fv_contint = [
            (mean(path_payoff) + 1.96 * mult * np.std(path_payoff, ddof=1) / np.sqrt(self.params.num_of_paths))
...
class EuropeanDigitalAnalyticPricer:
    ...
    def greeks(self) -> Dict[str, float]:
        ...
        rho = -T * P * np.exp(-r * T) * norm.cdf(d2) if self.contract.option_type == "call" else -T * P * np.exp(
            -r * T) * norm.cdf(-d2)
